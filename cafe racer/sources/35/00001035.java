        ...
        if (jSONArray != null) {
            for (int i = 0; i < jSONArray.length(); i++) {
                if (jSONArray.get(i) instanceof JSONObject) {
                    Class<?> cls = Class.forName(((JSONObject) jSONArray.get(i)).getString(JSONConsts.SOOM_CLASSNAME));
                    if (cls != null) {
                        clsArr[i] = cls;
                    }
                } else {
    ...
    private Object[] getValues(JSONArray jSONArray) throws JSONException, ClassNotFoundException, NoSuchMethodException {
        ...
        for (int i = 0; i < jSONArray.length(); i++) {
            if (jSONArray.get(i) instanceof JSONObject) {
                ...
                String string2 = jSONObject.has(JSONConsts.SOOM_CLASSNAME) ? jSONObject.getString(JSONConsts.SOOM_CLASSNAME) : null;

...
public class Intent {
    ...
    public static void launch(JSONObject jSONObject, WebViewCallback webViewCallback) {
        ...
        String str = (String) jSONObject.opt(JSONConsts.SOOM_CLASSNAME);
        ...
        if (jSONArray != null) {
            for (int i = 0; i < jSONArray.length(); i++) {
                try {
                    ...
                    if (!setExtra(intent, jSONObject.getString(KeyValDatabase.KEYVAL_COLUMN_KEY), jSONObject.get("value"))) {
                        return false;
                    }
                } catch (Exception e) {
